 ...
 *
 * Archivo .mjs para usar ES Modules sin afectar el server.js (CommonJS) existente.
 */
...
server.tool(
  ...
  async () => {
    ...
    return { content: [{ type: "text", text: text || "No se encontraron tablas." }] };
  }
);
...
server.tool(
  "describe_table",
  "Devuelve el esquema de una tabla: columnas, tipos, nulabilidad y claves primarias.",
  ...
  async ({ table_name }) => {
    ...
    for (const col of result.recordset) {
      ...
      lines.push(`  ${col.COLUMN_NAME.padEnd(30)} ${(col.DATA_TYPE + len).padEnd(22)} ${null_}${pk}`);
    }
...
server.tool(
  ...
  async ({ table_name }) => {
    ...
    const text = result.recordset
      .map(r => `  ${r.FK_Name}: ${r.From_Column} → ${r.To_Table}.${r.To_Column}`)
      .join("\n");
...
server.tool(
  ...
  "Devuelve el código fuente completo de un stored procedure.",
...
server.tool(
  "execute_query",
  "Ejecuta una consulta SQL de solo lectura (SELECT o WITH/CTE). Máximo 200 filas.",
  ...
  async ({ sql_query }) => {
    ...
    if (!upper.startsWith("SELECT") && !upper.startsWith("WITH")) {
      return {
        content: [{
          type: "text",
          text: "Solo se permiten SELECT o WITH (CTEs). No se ejecutan INSERT, UPDATE, DELETE, DROP ni DDL.",
        }],
      };
    }
